function [results,HRF_d_gen] = run_MatGenerateRandHRF(anev_params,dcv_params,HRF_mu,HRF_sigma)
...
FO = anev_params.FO;
FA = anev_params.FA;
HRF_d = anev_params.HRF_d; %%Used in KERgen but not for the actual convolution
...
Kgen = numel(KERgen);  %%Need this to compute simulation length... KEEP!!!!
...
if hrfOn 
    %%Convolve all at once (using canonical HRF sampled from Normal
    %%distribution N(mu,sigma))
    HRF_d_gen = normrnd(HRF_mu,HRF_sigma);
    BLDgen = convolve_anev_roi_hrf(NEVgen,FG,HRF_d_gen);
else
